...
The recommended clickjacking protection is to incorporate the `frame-ancestors` directive in the CSP. The `frame-ancestors 'none'` functions similarly to `X-Frame-Options: deny`, `frame-ancestors 'self'` similarly to `X-Frame-Options: sameorigin`, `frame-ancestors normal-website.com` functions similarly to `X-Frame-Options: allow-from https://normal-website.com`.
