...
Some website forms allow form inputs to be pre-populated via GET parameters prior to submission (i.e., we don't need to trick the user into filling out the form, we just inject our own parameters into the request and get the victim to click the submit button). Others require actual text which we'll need to trick the user 